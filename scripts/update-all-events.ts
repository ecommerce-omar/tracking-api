...
async function updateAllEvents() {
  try {
    console.log('🚀 Iniciando atualização de eventos para TODOS os trackings (incluindo entregues)...\n');

    const trackingRepository = container.getRepository<ITrackingRepository>('TrackingRepository');

    // Buscar todos os trackings sem exceção
    const allTrackings = await trackingRepository.findAll();
